...
2. **`src/lib/access.ts`** holds the freemium model. `requireAuth()` gates pages on login only (not subscription), so free users can reach the dashboard. `getUsage(supabase, userId)` returns `{ subscribed, used, remaining, canExtract }` — `used` is the count of the user's `claims` rows, and a non-subscribed user gets `FREE_EXTRACTION_LIMIT` (5) extractions. The paywall is enforced **server-side** in `POST /api/extract-claim`, which calls `getUsage` and returns HTTP 402 (`code: "limit_reached"`) before calling Gemini once the limit is hit. The dashboard and `ClaimExtractor` surface the remaining count and a Subscribe link, but the 402 is the real gate.
